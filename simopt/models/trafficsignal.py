...
class TrafficLight(Model):
    ...
    def __init__(self, fixed_factors={}):
        ...
        self.specifications = {
            ...
            "transition_probs": {
                ...
                "datatype": list,
                "default": [0.2, 0.1, 0.3, 0.1, 0.1, 0.2, 0.3, 0]
            },
            ...
            "redlight_arteries" : {
                "description": "The length of redlight duration of artery roads", 
                "datatype": list,
                "default" : [10, 10, 10, 10]
            },
            
            "redlight_veins" : {
                "description": "The length of redlight duration of vein roads", 
                "datatype": float,
                "default" : [20, 20, 20, 20]
            }
        }
    ...
    def check_offset(self):
        constrain1 = (max(self.factors["offset"][0], self.factors["offset"][4]) <= self.factors["offset"][2]) and (max(self.factors["offset"][1], self.factors["offset"][5]) <= self.factors["offset"][3]) and (max(self.factors["offset"][6], self.factors["offset"][10]) <= self.factors["offset"][8]) and (max(self.factors["offset"][7], self.factors["offset"][11]) <= self.factors["offset"][9])
        if constrain1 == False:
            return False
    ...
    def check_transition_probs(self):
        return all([transition_prob >= 0 for transition_prob in self.factors["transition_probs"]])
    ...
    def replicate(self, rng_list):
        ...
        #transition matrix
        p1, p2, p3, p4, p5, p6, p7, p8 = self.factors["transition_probs"]
        transition_matrix =  np.array([
                            [0 , p1, p5, 0 , p7, p3, p5,0],
                            [p1, 0 , p5, 0 , p3, p7, p5, 0],
                            [0 , 0,  0,  0 , 0 , 0 , 0 , 0],
                            [p6, p6, p2, 0 , p6, p6, p4, 0],
                            [p7, p3, p5, 0 , 0 , p1, p5, 0],
                            [p3, p7, p5, 0 , p1, 0 , p5, 0],
                            [0 , 0 , 0 , 0,  0 , 0 , 0 , 0],
                            [p6, p6, p4, 0 , p6, p6, p2, 0]])
        ...
        """
        Find greelight duration based on interval and redlight duration
        """ 
        greenlight_veins = list()
        greenlight_arteries = list()
        for ind in range(4):
            intersection = intersections[ind]
            r1 = intersection.verticalroads[0].roadid
            r5 = intersection.verticalroads[1].roadid
            r3 = intersection.horizontalroads[0].roadid
            f1 = self.factors["offset"][r1]
            f3 = self.factors["offset"][r3]
            f5 = self.factors["offset"][r5]
            v = min(self.factors["redlight_arteries"][ind]+f1-f3, self.factors["redlight_arteries"][ind]+f5-f3)
            greenlight_veins.append(v)
            a = v+self.factors["redlight_veins"][ind]-self.factors["redlight_arteries"][ind]
            greenlight_arteries.append(a) 
        print("offset:", self.factors["offset"])
        print("green_light artery vein:", greenlight_arteries, greenlight_veins)
        ...
        for roadid in range(12):
            ...
            print("road", roadid, " schedule: ", road.schedule, " status: ", road.status)
        ...
        def update_road_lights(t, roads): 
            ...
            for road in nextlightlocation:
                ...
                #print('Road', road.roadid, 'is changed to:', road.status, 'next chang: ',road.nextchange)
...
class MinWaitingTime(Problem):
    ...
    def __init__(self, name="TRAFFICCONTROL-1", fixed_factors={}, model_fixed_factors={}):
        ...
        self.lower_bounds = (0,)*12
        self.upper_bounds = (min(self.model.factors["redlight_arteries"]+self.model.factors["redlight_veins"])-1,)*12
